...
def data_treatement():
    ...
    for line in data[1:]:
        ...
        action = {
            "name": action_data[0],
            "price": float(action_data[1]),
            "percentage_benefits": float(action_data[2].strip().replace("%", "")),
            "euros_benefits": float(action_data[1]) * float(action_data[2].strip().replace("%", "")) / 100,
        }
...
def sac_a_dos(actions, budget):
    ...
    for action in actions:
        # Conversion du prix en int pour l'utiliser dans range et comme index
...
csv_path = "excel/dataset2_Python+P7.csv"
